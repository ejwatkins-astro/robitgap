...
This script contains the functions needed to perform the 1 iteration of the
robust iterative PCA technique in Budavari et al. 2009 (MNRAS 394, 1496–1502).
...
Wrappers perform a single iteration of the robust PCA which updates
...
Defining some parameters
    ...
    'Delta' is the breakdown point (between 0 to 0.5). This controls the
        robustness. The lower this value, the more susceptible the
    ...
    'SVD' or singular value decomposition factorises a Matrix, A into 3 new
        matrices, U, W, V^T. This factorisation happens to be related to the
        eigen-system of the original matrix, where U are the eigenvectors
        of A.A^T, and V are the eigenvectors of A^T.A. W are the singular
...
Notes:
    ...
    VW IDL implementation, this was not implemented, which essentially  means
...
def cauchy_like_function(t, c_sq=0.787**2):
    """
    Cauchy functions are bounded functions. Function defined in Budavari
    et al. 2009 in eq. (23).
    ...
    c_sq : float, optional
        ...
        (the strictness). The default is 0.787**2.
...
def derivate_of_cauchy_like_function(t, c_sq=0.787**2):
    """
...
def weight_star(t, c_sq=0.787**2, robust_function=cauchy_like_function):
    """
    ...
    robust_function : function, optional
        Function used to down-weight outliers. The default is cauchy_like_function.
...
def reconstruct_observation(observation_vector, eigen_vector_matrix):
    ...
    observation_vector : array_like
        A vector or matrix containing the centred data
    eigen_vector_matrix : 2d array_like matrix
        ...
        Dimensions: (length of each spectra, number of eigen vectors)
...
def get_vqu_coeficents(robust_derivative, mag_residuals_sq):
    """
    This function calculates the coefficients for the running weights
    q, u, and v, given in Budavari et al 2009 as eq. (20), (21) and (22)
    of Budavari et al. 2009 respectively.

    Parameters
    ----------
    robust_derivative : float
        The robust derivates evaluated at t
    ...
    -------
    robust_derivative : float
        Weight for the covariance matrix
...
def update_vqu(vqu_prev, weights, alpha):
    """
    Calculates the new values for q, u and v: eq. (20), (21) and (22)
    of Budavari et al. 2009 respectively.
    ...
    alpha : float
        'The forget' parameter. Value between 0 to 1. Controls how long
...
def get_gammas(vqu, vqu_prev, alpha):
    ...
    by the properties of the statistic.
...
def update_scale_sq(scale_sq_prev, mag_residuals_sq, gamma3,
                    delta=0.5, c_sq=0.787**2, robust_function=cauchy_like_function):
    """
    ...
    mag_residuals_sq : float
        The residual representing the current spectrum
    gamma3 : float
        The iterative for the scale equation
    ...
    c_sq : float, optional
        ...
        robust function. The default is 0.787**2.
    robust_function : function, optional
...
def update_weighted_mean(mean_prev, observation_vector, gamma1):
    """
    Calculates the new estimate of the location (i.e the mean): eq. (17)
...
def get_new_a(scale_sq_new, observation_vector, mag_residuals_sq, gamma2):
    """
    a is the vector appended onto the end of the eigen-system matrix A.
    ...
    mag_residuals_sq : mag_residuals_sq : float
        The residual representing the current spectrum
    gamma2 : float
        The iterative for the eigen-system equation
...
def get_A_current(eigen_vector_matrix, eigen_values, gamma2):
    """
    Calculates the current summary of the eigen-system. eq. (8)
    in Budavari et al 2009.
    ...
    A : 2d array_like matrix
        Matrix containing the summary of the eigen-system
        Dimensions: (length of each spectra, number of eigen vectors)
...
def get_A_new(A, a_next):
    """
    Calculates the new summary of the eigen-system. Is eq. (9) appended
    onto the end of the matrix calculated in eq. (8) in Budavari et al 2009.
    ...
    A_new : 2d array_like matrix
        Matrix containing the new summary of the eigen-system
        Dimensions: (length of each spectra, number of eigen vectors + 1)
...
def do_SVD_of_AtA(A_new):
    """
    Calculates the SVD of a matrix transpose times original matrix B.
    A * eigenvectors of B are the eigen vectors of the covariance matrix, C.
    But because B has a much smaller size than the covariance matrix,
    the speed of the SVD is massively improved.
    ...
    #B is symmetric square matrix, therefore singular values = eigenvalues
...
def update_eigen_system(eigen_vectors_B, singular_values_B, A_new, No_of_vectors):
    """
    Calculates the new eigen-system of the data covariance matrix using the
    matrix A, which is a summary of the eigen-system:
    Proof:
        Covariance matrix:
            C = A.A^T
        Columns of A are the eigen basis. Get B:
            B = A^T.A
        B is a square matrix whose size is equal to the number of
...
def normalise_filled_data_vector(spectra, filled_spectra, normalisation, where_bad):
    ...
    normalisation : array_like
        Array containing the normalisation values
    where_bad : array_like
        mask where the data is bad
    Returns
    -------
...
def get_filled_observation_vector(new_spectra, eigen_vectors, mean_prev, error_array):
    """
    Wrapper function of the relevant functions needed to get the
    observation vector where bad values are replaced with PCA
...
def PCA_from_residuals(eigen_system_dict, residuals, observation_vector, alpha, delta=0.5,
    ...
    eigen_system_dict : dictionary
        ...
        scale squared (sigma squared) and robust weights (vqu).
    """
    #for now, kept the same dictionary names as VW ILD implementation
    eigen_vectors = eigen_system_dict['U']     #eigenvectors as (nbin,nvec) array
