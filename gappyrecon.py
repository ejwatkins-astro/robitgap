...
This script contains the vectorised functions for calculating principle
...
we can use the expectation values (reconstructed data using eigen system)
...
are no longer entirely orthogonal.
...
of w are set to 1 and are these values are renormalised at the end
...
on the reconstructed data (called `N`, hence why vwpca_normgappy
has the comment: solve partial chi^2/partial N = 0)
...
x is the data vector (or a matrix with dimensions [Length of data, 1])
m is the data vector (or a matrix with dimensions [Length of data, 1])
a is a the pc scores vector (or a matrix with dimensions [number of eigen, 1])
...
WV call this `Fpr`
m^T*m is a windowed dot product of mean. Is scalar. VW call this `Mpr`

Want to eliminate N: Can multiply dX_sq/da by x^T*m=Fpr and multiply
...
Now we can subtract them from each other, which eliminates Fpr*F. Now all terms
have an `N`, which we can eliminate, leaving use with only `a`. We
...
def get_ammended_weigthed_correlation_matrix(mean_dot_data, weighted_eigen_transpose_by_eigen, weighted_scores, mean_scores):
    ...
    matrix has an amended result given here.
...
def get_ammended_weighted_scores(mean_dot_mean, weighted_scores, mean_dot_data, mean_scores):
    ...
    new_weighted_scores : array_like
        weighted PC scores amended by the normalisation
        Dimensions: [number of data vectors, number of eigen vectors]
...
def get_pc_scores(inverted_correlation_matrix, weighted_scores):
    ...
    This estimates the pc scores that account for gaps. The cost is
    that when the scores are used to fill data gaps, the data is no longer
    fully orthogonal since we are using the eigen system within the data itself
...
def run_normgappy(error_array, data, mean_array, eigen_vectors, verbose=False, return_run_mask=False):
    """
    ...
    Inputted data and errors need to have two dimensions as a result
    ...
    return_run_mask : bool, optional
        Whether to return mask where reconstruction performed on. The default 
        is False.
    ...
    Optional
    --------
    return_run_mask : bool
        If True, the data mask used to remove all bad spectra (good=True,
        bad=False) is returned. The default is False.
    ...
    data, errors = util.check_and_make_data_finite(data, error_array, verbose)

    #This removes the data vector (spectrum) if it only contains bad values
    good_data_vector, good_mask = where_data_all_good(error_array)
    ...
    if return_run_mask:
        return principle_comp_scores, normalisation, good_mask
    else:
